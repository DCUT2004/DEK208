class ArtifactMissionMightyLightning extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.MightyLightningActive || M2Inv.MightyLightningActive || M3Inv.MightyLightningActive)
			{
			...
			if (Inv.MightyLightningComplete)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.MightyLightningActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.MightyLightningActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.MightyLightningActive = True;
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionMightyLightningPickup'
     IconMaterial=Texture'MissionsTex4.WeaponMissions.MissionLightningGun'
