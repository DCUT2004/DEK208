class DruidDefenseSentinelControllerCrimbo extends Controller
	config(UT2004RPG);
...
function DoHealing()
{
	...
	Local DruidDefenseSentinelCrimbo DefPawn;
	Local float NumHelped;
	Local HealableDamageInv HDInv;
	local Mutator m;
	
	if (Pawn == None || Pawn.Health <= 0 || DruidDefenseSentinelCrimbo(Pawn) == None)
	    return;
	DefPawn = DruidDefenseSentinelCrimbo(Pawn);
...
function Timer()
{
	...
	if (PlayerSpawner == None || PlayerSpawner.Pawn == None || Pawn == None || Pawn.Health <= 0 || DruidDefenseSentinelCrimbo(Pawn) == None)
...
defaultproperties
{
     ...
     ShieldEmitterClass=Class'DEKRPG208.GoldBoltEmitter'
     HealthEmitterClass=Class'DEKRPG208.LightningBeamEmitter'
     AdrenalineEmitterClass=Class'DEKRPG208.LightningBoltEmitter'
