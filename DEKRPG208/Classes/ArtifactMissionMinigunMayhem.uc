class ArtifactMissionMinigunMayhem extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.MinigunMayhemActive || M2Inv.MinigunMayhemActive || M3Inv.MinigunMayhemActive)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.MinigunMayhemActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.MinigunMayhemActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.MinigunMayhemActive = True;
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionMinigunMayhemPickup'
     IconMaterial=Texture'MissionsTex4.WeaponMissions.MissionMinigun'
