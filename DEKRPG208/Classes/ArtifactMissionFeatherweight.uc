class ArtifactMissionFeatherweight extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.FeatherweightActive || M2Inv.FeatherweightActive || M3Inv.FeatherweightActive)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.FeatherweightActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.FeatherweightActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.FeatherweightActive = True;
...
static function string GetLocalString(optional int Switch, optional PlayerReplicationInfo RelatedPRI_1, optional PlayerReplicationInfo RelatedPRI_2)
{
	...
	if (Switch == 4000)
		...
		return "Remain in the air without using boots of flight or a translocator!";
}
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionFeatherweightPickup'
     IconMaterial=Texture'MissionsTex4.MiscellaneousMissions.FeatherWeightMission'
