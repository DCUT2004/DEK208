class ArtifactMissionHalt extends ArtifactMission
		config(UT2004RPG);
		
var config float NullTimer;
...
function Activate()
{
	...
	NInv = NullEntropyInv(Instigator.FindInventoryType(class'NullEntropyInv'));
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.HaltActive || M2Inv.HaltActive || M3Inv.HaltActive)
			{
			...
			{
				if (M1Inv.stopped && NInv == None)
				{
					M1Inv.Stopped = False;
					M1Inv.MissionXP = XPReward;
					M1Inv.MissionGoal = MissionGoal;
					M1Inv.HaltActive = True;
					M1Inv.MissionName = default.ItemName;
					M1Inv.SetTimer(M1Inv.CheckInterval, True);
					NInv = Instigator.spawn(class'NullEntropyInv');
					NInv.giveTo(Instigator);
					NInv.Lifespan = (NullTimer);
				}
				else if (M2Inv.stopped && NInv == None)
				{
					M2Inv.Stopped = False;
					M2Inv.MissionXP = XPReward;
					M2Inv.MissionGoal = MissionGoal;
					M2Inv.HaltActive = True;
...
static function string GetLocalString(optional int Switch, optional PlayerReplicationInfo RelatedPRI_1, optional PlayerReplicationInfo RelatedPRI_2)
{
	...
	if (Switch == 4000)
		...
		return "Survive while frozen!";
	else if (Switch == 8000)
		return "Cannot activate while already nulled!";
}

defaultproperties
{
	 NullTimer=300.00	//How long the Null should last. Null will destroy when mission is completed or forfeited
     XPReward=50
     MissionGoal=30
     Description="Survive while frozen!"
     PickupClass=Class'DEKRPG208.ArtifactMissionHaltPickup'
     IconMaterial=Texture'MissionsTex4.MiscellaneousMissions.HaltMission'
     ItemName="Halt!"
}
