class ArtifactMissionAngerManagement extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.AngerManagementActive || M2Inv.AngerManagementActive || M3Inv.AngerManagementActive)
			{
			...
			if (Inv.AngerManagementComplete)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.AngerManagementActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.AngerManagementActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.AngerManagementActive = True;
...
defaultproperties
{
     XPReward=50
     MissionGoal=1000
     PickupClass=Class'DEKRPG208.ArtifactMissionAngerManagementPickup'
     IconMaterial=Texture'MissionsTex4.MagicWeaponMissions.AngerManagementMission'
