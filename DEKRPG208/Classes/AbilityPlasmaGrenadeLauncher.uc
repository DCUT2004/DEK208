class AbilityPlasmaGrenadeLauncher extends CostRPGAbility
	config(UT2004RPG) 
	abstract;

// these config variables just affect the server so ok
var config Array< String > AGGLWeaponOne;
...
static function ModifyPawn(Pawn Other, int AbilityLevel)
{
	...
	if (LoadedInv != None)
	{
		if(LoadedInv.bGotAGGL && LoadedInv.LAGGLLevel == AbilityLevel)
			return;
	}
	else
	{
	...
	LoadedInv.bGotAGGL = true;
	OldLevel = LoadedInv.LAGGLLevel;		// keep old level so only add new weapons
	LoadedInv.LAGGLLevel = AbilityLevel;

	if(Other.Role != ROLE_Authority)
		return;

	if (OldLevel < 1)
		for(x = 0; x < default.AGGLWeaponOne.length; x++)
			giveWeapon(Other, default.AGGLWeaponOne[x], AbilityLevel, RPGMut);
...
defaultproperties
{
     AGGLWeaponOne(0)="DEKWeapons208.PlasmaGrenadeLauncher"
