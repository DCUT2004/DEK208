...
function Activate()
{
	     ...
	     && VSize(HitPawn.Location - StartTrace) < MaxRange && HitPawn.Controller.bGodMode == False)
	{
		...
		HitPawn.TakeDamage(DamageToDo, Instigator, HitPawn.Location, vect(0,0,0), class'DamTypeIceBeam');
		...
		if (!RunningTriple)
			ataken = (HealthTaken*AdrenalineUsage) / (AdrenalineRequired * UDamageAdjust);	    // take less adrenaline
		else
			ataken = (HealthTaken*AdrenalineUsage) / AdrenalineRequired;	    				// take single damage adrenaline
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactLightningBeamPickup'
     IconMaterial=Texture'DEKRPGTexturesMaster207P.Artifacts.LightningBeamIcon'
     ItemName="Ice Beam"
}
