class LetterUInv extends Inventory
	config(UT2004RPG);
		...
		if (PawnOwner != None && PawnOwner.Health > 0)
		{
			for ( C = Level.ControllerList; C != None; C = C.NextController )
				if ( C != None && C.Pawn != None && C.Pawn.Health > 0 && C.SameTeamAs(PawnOwner.Controller) && !C.Pawn.IsA('Monster') )
				{
					...
					{
						U = LetterUInv(P.FindInventoryType(class'LetterUInv'));
						if (U == None)
						{
							U = spawn(class'LetterUInv', P,,, rot(0,0,0));
							U.GiveTo(P);
						}
					}
				}
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactLetterUPickup'
