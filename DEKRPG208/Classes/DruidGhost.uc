...
// ULTIMA REMOVED.  Shouldn't be needed in RPGDeathAbility.
	...
	MMPI = MissionMultiplayerInv(Killed.FindInventoryType(class'MissionMultiplayerInv'));
	if (MMPI != None && !MMPI.MasterMMPI)	//Just in case the mission finishes/times up while ghosting
	{
		MMPI.StopEffect();
	}
...
defaultproperties
{
     ...
     ExcludingAbilities(2)=Class'DEKRPG208.AbilityNecroGhost'
     AbilityName="Ghost"
