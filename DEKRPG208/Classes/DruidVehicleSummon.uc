...
function bool SpawnIt(TranslocatorBeacon Beacon, Pawn P, EngineerPointsInv epi)
{
	...
	if (!CheckSpace(SpawnLoc,450,300))
	{
	...
	else if (DEKRaptor(NewVehicle) != None)
	    DEKRaptor(NewVehicle).SetPlayerSpawner(Instigator.Controller);
	else if (DEKTank(NewVehicle) != None)
	    DEKTank(NewVehicle).SetPlayerSpawner(Instigator.Controller);
	else if (DEKLynxVehicle(NewVehicle) != None)
	    DEKLynxVehicle(NewVehicle).SetPlayerSpawner(Instigator.Controller);
...
defaultproperties
{
     IconMaterial=Texture'DEKRPGTexturesMaster207P.Artifacts.SummonVehicleIcon'
     ItemName=""
}
