class ArtifactMissionShockSlaughter extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.ShockSlaughterActive || M2Inv.ShockSlaughterActive || M3Inv.ShockSlaughterActive)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.ShockSlaughterActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.ShockSlaughterActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.ShockSlaughterActive = True;
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionShockSlaughterPickup'
     IconMaterial=Texture'MissionsTex4.WeaponMissions.MissionShockRifle'
