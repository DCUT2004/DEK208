class LetterOInv extends Inventory
	config(UT2004RPG);
		...
		if (PawnOwner != None && PawnOwner.Health > 0)
		{
			for ( C = Level.ControllerList; C != None; C = C.NextController )
				if ( C != None && C.Pawn != None && C.Pawn.Health > 0 && C.SameTeamAs(PawnOwner.Controller) && !C.Pawn.IsA('Monster') )
				{
					...
					{
						O = LetterOInv(P.FindInventoryType(class'LetterOInv'));
						if (O == None)
						{
							O = spawn(class'LetterOInv', P,,, rot(0,0,0));
							O.GiveTo(P);
						}
					}
				}
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactLetterOPickup'
