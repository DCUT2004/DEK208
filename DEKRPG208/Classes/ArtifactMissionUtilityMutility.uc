class ArtifactMissionUtilityMutility extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.UtilityMutilityActive || M2Inv.UtilityMutilityActive || M3Inv.UtilityMutilityActive)
			{
			...
			if (Inv.UtilityMutilityComplete)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.UtilityMutilityActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.UtilityMutilityActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.UtilityMutilityActive = True;
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionUtilityMutilityPickup'
     IconMaterial=Texture'MissionsTex4.WeaponMissions.MissionUtility'
