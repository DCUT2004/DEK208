...
function Activate()
{
	...
	while (C != None)
	{
		...
		if ( C.Pawn != None && C.Pawn != Instigator && C.Pawn.Health > 0 && !C.SameTeamAs(Instigator.Controller)
		     && VSize(C.Pawn.Location - Instigator.Location) < TargetRadius && FastTrace(C.Pawn.Location, Instigator.Location) && C.bGodMode == False && !C.Pawn.IsA('HealerNali') && !C.Pawn.IsA('MissionCow') && !C.Pawn.IsA('MissionBalloon') && PhantomDeathGhostInv(C.Pawn.FindInventoryType(class'PhantomDeathGhostInv')) == None)
		{
	...
	if ((MostHealth > 0) && (BestC != None) && (BestC.Pawn != None))
	{
		...
		DamageToDo = min(MaxDamage,AdrenalineRequired * (Instigator.Controller.Adrenaline/AdrenalineUsage));
		...
		if (!RunningTriple)
			Instigator.Controller.Adrenaline -= (HealthTaken*AdrenalineUsage) / (AdrenalineRequired * UDamageAdjust);	    // take less adrenaline
		else
			Instigator.Controller.Adrenaline -= (HealthTaken*AdrenalineUsage) / AdrenalineRequired;	    				// take single damage adrenaline
...
defaultproperties
{
     HitEmitterClass=Class'DEKRPG208.LightningBoltEmitter'
     ...
     PickupClass=Class'DEKRPG208.ArtifactLightningBoltPickup'
     IconMaterial=Texture'DEKRPGTexturesMaster207P.Artifacts.LightningBoltIcon'
