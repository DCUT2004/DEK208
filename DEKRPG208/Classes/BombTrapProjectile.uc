class BombTrapProjectile extends Projectile
	config(DEKWeapons);
...
simulated function Destroyed()
{
    ...
    if ( BombTrap(Owner) != None)
    	BombTrap(Owner).CurrentMines--;
	if ( Beacon != None )
		Beacon.Destroy();
...
simulated function PostNetBeginPlay()
{
	...
	{
		Beacon = spawn(class'BombTrapBeacon', self);
...
simulated function ProcessTouch( actor Other, vector HitLocation )
{
	...
	if (Base != None && P != None && P != Instigator && P.Health > 0 && P.IsA('Monster') && InstigatorController != None && !P.Controller.SameTeamAs(InstigatorController) && Inv == None && Vehicle == None && !P.IsA('HealerNali') && !P.IsA('MissionCow') && bCanDetonate)
	{
		...
		{
			FlakExplode(Location, vect(0,0,1));
			bCanDetonate = False;
			if (Beacon != None)
				Beacon.Destroy();
...
simulated function FlakExplode(vector HitLocation, vector HitNormal)
{
	local Actor A,B,C;
	...
	if ( Role == ROLE_Authority )
	{
		...
		A = Spawn(class'FlakExplosion',,, HitLocation + HitNormal*20, rotator(HitNormal));
		if (A != None)
		{
			A.RemoteRole = ROLE_SimulatedProxy;
		}
		B = Spawn(class'FlashExplosion',,, HitLocation, rotator(vect(0,0,1)));
		if (B != None)
		{
			B.RemoteRole = ROLE_SimulatedProxy;
		}
		C = Spawn(class'RocketSmokeRing',,, HitLocation, rotator(vect(0,0,1)));
		if (C != None)
		{
...
defaultproperties
{
     ...
     MyDamageType=Class'DEKRPG208.DamTypeBombTrap'
     ImpactSound=Sound'MenuSounds.select3'
     ExplosionDecal=Class'DEKRPG208.BombTrapMark'
     DrawType=DT_StaticMesh
     StaticMesh=StaticMesh'DEKStaticsMaster207P.Meshes.BombTrap'
