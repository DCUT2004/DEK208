...
struct CopyXPValue
{
	...
	var int PlayerClass;       // 0 none, 1 WM, 2 AM, 3 MM, 4 Eng, 5 Gen, 6 Warrior, 7 Necro
	var int XPGained;
	var string SubClass;
};
...
simulated function DrawSubClass (Canvas Canvas, string text, float x, float y)
{
	...
	Canvas.DrawColor = SubClassColor;
...
simulated function DrawXP (Canvas Canvas, int PlayerIndex, int PTypeXPos, int XPXPos, int YPos, int YPlayerBox, int YL)
{
	...
	for (j=0; j<CopyXPs.length; j++)
		if (CopyXPs[j].PlayerName == PRIArray[PlayerIndex].PlayerName)
		{
			...
			else if (CopyXPs[j].PlayerClass == 6)
			{	//Warrior
				Canvas.DrawTile(Material'HUDContent.Generic.HUD',YL,YL,169,172,76,36);
				if (CopyXps[j].SubClass != "" && CopyXps[j].SubClass != "None")
					DrawSubClass (Canvas, CopyXps[j].SubClass, PTypeXPos + (YL/2), NewYPos);
				Canvas.DrawColor = WarBrownColor;
			}
			else if (CopyXPs[j].PlayerClass == 7)
			{	//Necromancer
				Canvas.DrawTile(Material'DEKRPGTexturesMaster207P.NecroIcons.NecroClassIcon',YL,YL,1,1,64,64);
				if (CopyXps[j].SubClass != "" && CopyXps[j].SubClass != "None")
					DrawSubClass (Canvas, CopyXps[j].SubClass, PTypeXPos + (YL/2), NewYPos);
...
simulated event UpdateScoreBoard(Canvas Canvas)
{
	...
	XPText = "XP";
	 ...
	 {
	 	iClientCheckCount++;
	 	if (iClientCheckCount > 25)		//just to stop check putting too much load on if problems
	 	{
	 		iClientCheckCount = 0;
			if (Owner != None)
			{
...
defaultproperties
{
     AMOrangeColor=(G=50,R=235,A=255)
     WMPurpleColor=(B=204,G=4,R=109,A=255)
     MMBlueColor=(B=234,G=178,R=104,A=255)
     GenGreenColor=(B=15,G=161,R=18,A=255)
     WarBrownColor=(B=66,G=71,R=61,A=255)
	 NecroRedColor=(B=0,G=0,R=105,A=255)
     SubClassColor=(B=255,G=109,R=184,A=255)
     iClientCheckCount=18
     AdminText="NERD"
     NotReadyText="IDLE"
     SkillLevel(0)="NOVICE"
