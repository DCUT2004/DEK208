class ArtifactMissionEmeraldShatter extends ArtifactMission
		config(UT2004RPG);
...
function Activate()
{
	...
	if ((Instigator != None) && (Instigator.Controller != None))
	{
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			if (M1Inv.EmeraldShatterActive || M2Inv.EmeraldShatterActive || M3Inv.EmeraldShatterActive)
			{
			...
			else
			{
				if (M1Inv.stopped)
				{
					...
					M1Inv.EmeraldShatterActive = True;
				...
				else if (M2Inv.stopped)
				{
					...
					M2Inv.EmeraldShatterActive = True;
				...
				else if (M3Inv.stopped)
				{
					...
					M3Inv.EmeraldShatterActive = True;
...
static function string GetLocalString(optional int Switch, optional PlayerReplicationInfo RelatedPRI_1, optional PlayerReplicationInfo RelatedPRI_2)
{
	...
	if (Switch == 4000)
		...
		return "Kill monsters spawned by the emerald invasion orb!";
}
...
defaultproperties
{
     ...
     PickupClass=Class'DEKRPG208.ArtifactMissionEmeraldShatterPickup'
     IconMaterial=Texture'MissionsTex4.MiscellaneousMissions.EmeraldShatter'
