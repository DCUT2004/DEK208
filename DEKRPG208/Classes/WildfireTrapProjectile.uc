class WildfireTrapProjectile extends Projectile
	config(DEKWeapons);
...
simulated function Destroyed()
{
    ...
    if ( WildfireTrap(Owner) != None)
    	WildfireTrap(Owner).CurrentMines--;
	if ( Beacon != None )
		Beacon.Destroy();
...
simulated function PostNetBeginPlay()
{
	...
	{
		Beacon = spawn(class'WildfireTrapBeacon', self);
...
simulated function Explode(vector HitLocation, vector HitNormal)
{
	...
	if ( Role == ROLE_Authority )
	{
		...
		A = Spawn(class'WildfireExplodeFX',,, Location, rotator(vect(0,0,1)));
		B = Spawn(Class'WildfireFlameFX',,,Location,rotator(HitNormal));
		C = Spawn(Class'WildfireSmoke',,,Location,rotator(HitNormal));
		if (A != None)
...
function BoomSound()
{
	self.PlaySound(sound'DEKRPG208.TurretSounds.SolarTurretAltFire',,500.00);
}
...
defaultproperties
{
     ...
     MyDamageType=Class'DEKRPG208.DamTypeWildfireTrap'
     ImpactSound=Sound'MenuSounds.select3'
     DrawType=DT_StaticMesh
     StaticMesh=StaticMesh'DEKStaticsMaster207P.Meshes.WildfireTrap'
