class DEKLoadedUtility extends CostRPGAbility
	config(UT2004RPG) 
	abstract;

// these config variables just affect the server so ok
var config Array< String > LoadedUtilityOne;
...
static function ModifyPawn(Pawn Other, int AbilityLevel)
{
	...
	if (LoadedInv != None)
	{
		if(LoadedInv.bGotLoadedUtility && LoadedInv.LUtilityAbilityLevel == AbilityLevel)
			return;
	}
	else
	{
	...
	LoadedInv.bGotLoadedUtility = true;
	OldLevel = LoadedInv.LUtilityAbilityLevel;		// keep old level so only add new weapons
	LoadedInv.LUtilityAbilityLevel = AbilityLevel;
...
// shouldn't.
	...
	if (OldLevel < 1)
		for(x = 0; x < default.LoadedUtilityOne.length; x++)
			giveWeapon(Other, default.LoadedUtilityOne[x], AbilityLevel, RPGMut);
...
defaultproperties
{
     ...
     PlayerLevelReqd(1)=1
     PlayerLevelReqd(2)=1
     ExcludingAbilities(0)=Class'DEKRPG208.DEKLoadedClassicShield'
