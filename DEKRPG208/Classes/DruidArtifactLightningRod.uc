	...
	function Timer()
	{
		...
		while (C != None)
		{
			...
			if ( C.Pawn != None && Instigator != None && C.Pawn != Instigator && C.Pawn.Health > 0 && !C.SameTeamAs(Instigator.Controller)
			     && VSize(C.Pawn.Location - Instigator.Location) < TargetRadius && FastTrace(C.Pawn.Location, Instigator.Location) && !C.Pawn.IsA('HealerNali') && !C.Pawn.IsA('MissionCow') && !C.Pawn.IsA('MissionBalloon') && PhantomDeathGhostInv(C.Pawn.FindInventoryType(class'PhantomDeathGhostInv')) == None )
			{
...
defaultproperties
{
     ...
     CostPerSec=1
     PickupClass=Class'DEKRPG208.DruidArtifactLightningRodPickup'
