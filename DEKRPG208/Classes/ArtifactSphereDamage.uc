...
#exec  AUDIO IMPORT NAME="SphereActivate" FILE="C:\UT2004\Sounds\SphereActivate.WAV" GROUP="ArtifactSounds"
#exec  AUDIO IMPORT NAME="SphereDeactivate" FILE="C:\UT2004\Sounds\SphereDeactivate.WAV" GROUP="ArtifactSounds"
...
function SetTeamDamage(vector CoreLocation)
{
	...
	while (C != None)
	{
		...
		if ( C.Pawn != None && vehicle(C.Pawn) == None && C.Pawn != Instigator && C.Pawn.Health > 0 && C.SameTeamAs(Instigator.Controller)
		     && VSize(C.Pawn.Location - CoreLocation) < EffectRadius && !C.Pawn.HasUDamage() && RedeemerWarhead(C.Pawn) == None && PhantomDeathGhostInv(C.Pawn.FindInventoryType(class'PhantomDeathGhostInv')) == None)
		{
			//if(HardCoreInv(C.Pawn.FindInventoryType(class'HardCoreInv')) == None || PhantomDeathGhostInv(C.Pawn.FindInventoryType(class'PhantomDeathGhostInv')) == None )
				// no got udamage is false so can't already have this set yet
				Inv = spawn(class'DamageInv', C.Pawn,,, rot(0,0,0));
				if(Inv != None)
				{
					Inv.bSphereActivated = True;
...
state Activated
{
	...
	{	local Vehicle V;
		...
		if ((Instigator != None) && (Instigator.Controller != None))
		{
			...
			SetTimer(0.5, true);
			Instigator.PlaySound(Sound'DEKRPG208.ArtifactSounds.SphereActivate',,200.00);
		}
	}
	function Timer()
	{
	...
	function EndState()
	{
		...
		if (Instigator != None)
		{
			Instigator.DisableUDamage();
			Instigator.SetOverlayMaterial(EffectOverlay, -1, true);
			Instigator.PlaySound(Sound'DEKRPG208.ArtifactSounds.SphereDeactivate',,200.00);
		}
		bActive = false;
...
defaultproperties
{
     ...
     CostPerSec=15
     PickupClass=Class'DEKRPG208.ArtifactSphereDamagePickup'
     IconMaterial=Texture'DEKRPGTexturesMaster207P.Artifacts.SphereDamage'
