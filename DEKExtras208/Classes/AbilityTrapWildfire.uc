class AbilityTrapWildfire extends CostRPGAbility
	config(UT2004RPG) 
	abstract;
...
static function ModifyPawn(Pawn Other, int AbilityLevel)
{
	...
	if (LoadedInv != None)
	{
		if(LoadedInv.bGotWildfireTrap && LoadedInv.WTrapAbilityLevel == AbilityLevel)
			return;
	}
	else
	{
	...
	LoadedInv.bGotWildfireTrap = true;
	OldLevel = LoadedInv.WTrapAbilityLevel;		// keep old level so only add new weapons
	LoadedInv.WTrapAbilityLevel = AbilityLevel;
	...
	// now add the new one.
	NewWeapon = Other.spawn(class'WildfireTrap', Other,,, rot(0,0,0));
	if(NewWeapon == None)
		return;
	...
	if (WildfireTrap(NewWeapon) != None)
		WildfireTrap(NewWeapon).MaxMines += (AbilityLevel - 1); //default is 3 for DC Server
}

defaultproperties
{
     ExcludingAbilities(0)=Class'DEKExtras208.AbilityTrapFrost'
