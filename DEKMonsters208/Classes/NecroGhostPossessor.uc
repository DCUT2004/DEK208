class NecroGhostPossessor extends Monster
...
var config float PossessRadius;
var NecroGhostPossessorInvasionOrb Orb;
...
function RangedAttack(Actor A)
{
	...
	else
	{
		...
		else
		{
			...
			if (rand(99) <= 15)
			{
				B = Instigator.spawn(class'NecroGhostPossessorLight', Instigator,, Instigator.Location, Instigator.Rotation);
				if (B != None)
				{
...
function SpawnOrb()
{
	...
	Orb = Spawn(class'NecroGhostPossessorInvasionOrb',Instigator,,Instigator.Location);
	GotoState('Teleporting');
	if (rand(99) <= 15)
	{
		A = Instigator.spawn(class'NecroGhostPossessorLight', Instigator,, Instigator.Location, Instigator.Rotation);
		if (A != None)
		{
...
simulated function PlayDying(class<DamageType> DamageType, vector HitLoc)
{
	...
	A = spawn(class'NecroGhostPossessorVanishFX', Self,, Self.Location, Self.Rotation);
	if (A != None)
		A.RemoteRole = ROLE_SimulatedProxy;
}
...
defaultproperties
{
     ...
     Mesh=SkeletalMesh'ThunderCrash.Mercfemale1'
     Skins(0)=FinalBlend'D-E-K-HoloGramFX.FullFB.HoloMaterial_2'
     Skins(1)=FinalBlend'D-E-K-HoloGramFX.FullFB.HoloMaterial_2'
